# 1. These two lines MUST be at the very top
import eventlet
...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'default_key_change_in_prod')

# 2. Initialize SocketIO with eventlet async mode
...
def generate_levels():
    ...
    for i in range(1, 101):
        ...
        levels[i] = {
            'level': i, 'difficulty': difficulty, 'grid_size': min(grid_size, 10),
...
class DotsBoxesGame:
    ...
    def check_boxes(self, line_type, i, j, player):
        completed = False
        # Helper to check a specific box coordinates
        def is_complete(r, c):
    ...
    # AI Helpers (Simplified for brevity)
...
def on_create(data):
    ...
    join_room(code)
    active_rooms[code]['players'][request.sid] = 1 # Host is P1
...
def on_multi_move(data):
    ...
    if game.current_player != player_num: return # Not your turn
...
def on_disconnect():
    for code, room in active_rooms.items():
        if request.sid in room['players']:
            ...
            # Clean up room logic could go here
